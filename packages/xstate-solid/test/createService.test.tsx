...
describe('createService hook', () => {
  ...
  it('should provide options for the service', () => {
    const Test = () => {
      const {options} = createService(fetchMachine, {
        execute: false
      });
  ...
  it('should merge machine context with options.context', () => {
    ...
    const Test = () => {
      const {state} = createService(testMachine, { context: { test: true } });
  ...
  it('should not spawn actors until service is started', (done) => {
    ...
    const Spawner = () => {
      const {state} = createService(spawnMachine);
  ...
  it('should be able to use an action provided outside of SolidJS', () => {
    ...
    const App = () => {
      const {send} = createService(machine);
      send({ type: 'EV' });
      return null;
    };
...
describe('useMachine (strict mode)', () => {
  ...
  it('delayed transitions should work when initializing from a rehydrated state', () => {
    ...
    try {
      ...
      const Test = () => {
        const {state, send} = createService(testMachine, {
